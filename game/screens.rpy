...
## Екран промови ###############################################################
##
## Показує діалог гравцеві. Приймає параметри «who» (ім'я персонажа, або
## None) та «what» (текст репліки).
##
...
screen say(who, what):
    ...
    ## Бічне зображення персонажа над текстом (не показується на мобільних —
    ## там немає місця).
    if not renpy.variant("small"):
        add SideImage() xalign 0.0 yalign 1.0


## Дозволяє стилізувати поле імені через об'єкт Character.
init python:
...
## Цей код гарантує, що екран "quick_menu" показується у грі, якщо гравець не приховав інтерфейс явно.
init python:
...
screen navigation():
    ...
    ## Трігер для визначення переходу до іншого підменю відмінного від головного меню
    ...
    vbox:
        ...
        ## Прив'язка до центру самого блока — фіксує позицію навігації незалежно від зміни ширини кнопок/шрифту.
        xanchor 0.5
        spacing 2
        ...
        ## Додатковий відступ кнопки вихід в залежності від вікна 
        if main_menu:
            null height 130
        else:
            null height 80
...
## Екран з назвою гри ##########################################################
##
## Параметр "blur" визначає чи потрібно блюрити текст якщо вибране підменю відмінне від головного меню 
## Використовується на тлі меню гри, де фон і так заблюрений.
...
## Екран ігрового меню #########################################################
##
## Спільна структура для about/save/load/preferences/history: фон,
## заголовок, навігація і рамка з вмістом (переданим через transclude).
##
## Параметри:
##   scroll       — None або "viewport" (чи потрібна прокрутка вмісту).
##   frame_xpos   — горизонтальна позиція вікна (None — стандартна).
##   frame_xsize  — ширина вікна (None — стандартна).
screen game_menu(title, scroll=None, yinitial=0.0, spacing=0,frame_xpos=None,frame_xsize=None):

    style_prefix "game_menu"

    ## Фон і заблюрена назва гри позаду вікна меню.
    add gui.main_menu_background:
    ...
    frame:
        ...
        hbox:
            ...
            ## Місце для розділу навігації (Порожнє, але потрібно).
            frame:
            ...
            frame:
                ...
                if scroll == "viewport":
                    ...
                    ## Кастомна прокрутка: viewport + окремий vbar
                    fixed:
                        viewport:
                            id "vp"
                            yinitial yinitial
                        ...
                        vbar:
                            ...
                            ## Позиція скролбару своя для кожного екрана.
                            if renpy.get_screen("about"):
                                xpos gui.vscroll_xpos_about
                            elif renpy.get_screen("history"):
                                xpos gui.vscroll_xpos_history
                            else:
                                xpos gui.vscroll_xpos_preferences
                            ypos 0
    ...
    add gui.return_button_cover_not_activated:
        xalign 0.161
        yalign 0.265
        ...
        vbox:
            ...
            hbox:
                ...
                vbox:
                    ...
                    label _("Швидкість авто. переходу")
...
##
## Екран "skip_indicator" показується, щоб вказати, що відбувається пропуск.
##
...
## Екран сповіщення ############################################################
##
## Екран сповіщення використовується для показу гравцеві повідомлення.
